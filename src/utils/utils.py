...
def plot_predictions_vs_reference(config, model, device, it, xyz_ref, u_lr, v_lr, w_lr, p_lr, u_ref, v_ref, w_ref, p_ref, px_ref, py_ref, pz_ref, mask_ref, mask_flat_ref, U_max, standardization_factors):
    ...
    if config.setup.include_pressure and not config.training.reference_gradients:
        ...
        plt.subplot(1, 3, 2)
        plt.title('Reference p')
        plt.imshow(p_ref[t_step*config.ref_temporal_factor, :, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis', vmin=14100)
        plt.colorbar()
    ...
    if config.training.reference_gradients:
        ...
        plt.subplot(1, 2, 1)
        plt.title('Reference p')
        plt.imshow(px_ref[t_step*config.ref_temporal_factor, :, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
        plt.subplot(1, 2, 2)
        plt.title('Predicted p')
        plt.imshow(px_pred[:, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
        ...
        plt.subplot(1, 2, 1)
        plt.title('Reference p')
        plt.imshow(py_ref[t_step*config.ref_temporal_factor, :, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
        plt.subplot(1, 2, 2)
        plt.title('Predicted p')
        plt.imshow(py_pred[:, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
        ...
        plt.subplot(1, 2, 1)
        plt.title('Reference p')
        plt.imshow(pz_ref[t_step*config.ref_temporal_factor, :, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
        plt.subplot(1, 2, 2)
        plt.title('Predicted p')
        plt.imshow(pz_pred[:, :, config.plot.z_slice*config.ref_spatial_factor], cmap='viridis')
        plt.colorbar()
