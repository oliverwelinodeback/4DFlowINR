...
def sample_from_gpu(config, xyz_train_gpu, xyz_collocation_gpu, xyz_boundary_gpu, uvw_train_gpu, mask_flat_gpu):
    ...
    if config["sample_collocation"]:
        if config["training"]["coll_points_per_batch"] is not None:
            coll_indices = torch.randint(
                low=0, 
                high=xyz_collocation_gpu.shape[0], 
                size=(config["training"]["coll_points_per_batch"],), 
                device=device
            )
