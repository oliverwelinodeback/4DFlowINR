...
def get_sweep_config():
    ...
    return {
        'name': f'NIR_Tunings_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': "Relative Error [Fluid]", 'goal': 'minimize'},
        'parameters': {
            'network.sigma_0': {'values': [0, 30]},
            'training.use_vector_potential': {'values': [False, True]},
            'training.use_physics_loss': {'values': [False, True]},
            'training.lr_decay_iter': {'values': [99999999, 1000]},
            'training.BFGS_lr': {'values': [1, 1e-1, 1e-2]},
            'training.iterations_before_BFGS': {'values': [99999, 6000, 4000]},
            'training.use_cosine': {'values': [True, False]},
            'training.sample_boundary': {'values': [True, False]},
        },
    }
...
def get_config(sweep_config=None):
    ...
    # Model 
    config.networks_folder = "../models/251014_tuning/"
    config.network_name = "HV01_WIRE" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.training.iterations_before_BFGS = 6000 #!#
    # Loss details
