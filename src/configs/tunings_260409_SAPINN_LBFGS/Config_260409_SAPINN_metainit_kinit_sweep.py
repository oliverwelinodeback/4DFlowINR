...
def get_sweep_config():
    """
    Meta-init K_initial sweep — Step 3a

    Finds the optimal SA start time for meta-initialized networks. Total iters=40k,
    itBFGS=10k fixed (repro best). No lr_decay — LBFGS handles step sizing.

    With meta-init the network residuals are structured from iter 0, so SA may
    benefit from firing earlier than the repro K_initial=10k. All sweep values
    are below itBFGS=10k, so SA always has Adam time before LBFGS takes over.
    ...
    return {
        'name': f'SAPINN_metainit_kinit_sweep_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'training.K_initial': {'values': [1000, 3000, 5000, 8000, 10000]},
        },
    }
...
def get_config(sweep_config=None):
    ...
    Base: repro_SA settings + MAML meta-init + itBFGS=10k (repro best).
    Only K_initial is swept. disable_lr_decay=True — no step decay.

    Sequence after this sweep:
      3b — itBFGS sweep with best K_initial fixed
      3c — iteration budget sweep with both K_initial and itBFGS fixed

    data_file fixed to ICAD48.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260409_SAPINN_metainit_kinit_sweep/"
    config.network_name = "260409_SAPINN_metainit_kinit_sweep"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # Optimizer — itBFGS=10k fixed (repro best); no lr_decay
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 99_999       # disabled
    ...
    config.training.iterations_before_BFGS = 10_000  # fixed — not swept here
    ...
    # K_initial swept via trainer.py readback (line 173)
    # ==========================================
    ...
    config.training.K_initial = 10_000   # default; overridden by sweep readback
