...
def get_sweep_config():
    """
    Sweep 4 — Short-budget + SA ablation on ICAD48

    Tests short training budgets (10k, 15k) combined with SA on/off and
    meta-init on/off. All settings encoded in run_id; trainer.py routing
    applies per-run overrides via the SWEEP4 dict.

    | Run | iters | itBFGS | K_initial | SA    | meta-init |
    |-----|-------|--------|-----------|-------|-----------|
    | A   | 10k   | 5k     | 3k        | True  | True      |
    | B   | 15k   | 10k    | 3k        | True  | True      |
    | C   | 10k   | 5k     | —         | False | True      |
    | D   | 15k   | 10k    | —         | False | True      |
    | E   | 10k   | 5k     | 5k        | True  | False     |
    | F   | 15k   | 10k    | 10k       | True  | False     |

    Questions:
    - A vs C, B vs D: does SA help with meta-init at short budgets?
    - E vs F: does short budget work without meta-init?
    - A/C vs E/F: is meta-init necessary for short budgets?

    Method: grid (6 runs, ~90 min each).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_sweep4_shortbudget_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'run_id': {'values': ["A", "B", "C", "D", "E", "F"]},
        },
    }
...
def get_config(sweep_config=None):
    """
    Sweep 4 — short-budget + SA ablation.

    Base config: ICAD48, repro_SA params, meta-init enabled, SA enabled.
    All per-run overrides (iterations, itBFGS, K_initial, SA, meta-init)
    are applied by trainer.py via the SWEEP4 routing dict using run_id.

    disable_lr_decay=True: no step decay in any run — LBFGS handles step sizing.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260409_SAPINN_sweep4_shortbudget/"
    config.network_name = "260409_SAPINN_sweep4"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # ==========================================
    # META-INIT — default True; overridden to False for runs E and F
    # ==========================================
    ...
    # ==========================================
    # TRAINING PARAMETERS
    # All per-run values overridden by trainer.py SWEEP4 routing
    # ==========================================
    config.training = ml_collections.ConfigDict()
    config.training.iterations = 15_000        # default (run B/D/F); overridden by run_id
    ...
    # Optimizer — all values overridden by run_id routing
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 99_999     # always disabled in sweep 4
    ...
    config.training.iterations_before_BFGS = 10_000  # default; overridden by run_id
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — default True; overridden to False for runs C and D
    # K_initial overridden per run_id via SWEEP4 routing
    # ==========================================
    ...
    config.training.K_initial = 10_000        # default; overridden by run_id
