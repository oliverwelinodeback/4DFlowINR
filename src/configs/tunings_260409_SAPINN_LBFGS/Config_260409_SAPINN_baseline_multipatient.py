...
def get_sweep_config():
    """
    SA-PINN baseline — multi-patient (true baseline)

    Exact repro_SA settings converted to a multi-patient sweep.
    No LBFGS (itBFGS=99_999), lr_decay at iter 25k, 40k total iters.
    repro_SA params: tau=0.02, beta=0.2, K=20, K_initial=10k, weight_clip=[6,0.2].

    Purpose: establish ground-truth performance on HV01, ICAD48, ICAD21 before
    testing LBFGS and meta-init variants.

    Method: grid (3 runs).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_baseline_multipatient_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-PINN true baseline — multi-patient.

    Identical to Config_260303_SAPINN_repro_SA.py with sweep=True and
    data_file overridden per-patient via trainer.py LR_ROUTING.

    No LBFGS, scheduler step-decay at 25k, 40k total iters.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260409_SAPINN_baseline_multipatient/"
    config.network_name = "260409_SAPINN_baseline_multipatient"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # Optimizer — no LBFGS, step decay at 25k (repro_SA baseline settings)
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 25_000
    config.training.lr_decay_factor = 0.5
    config.training.disable_lr_decay = False   # keep scheduler active
