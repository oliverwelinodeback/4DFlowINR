...
def get_sweep_config():
    """
    Sweep N — Non-meta-init 2x2 factorial (Phase 1: N1–N4)

    Isolates the individual and combined contributions of SA and LBFGS without
    meta-init. Fixed budget: 20k total iters, ICAD48 only.

    | Run | SA    | LBFGS | K_initial | itBFGS | Question              |
    |-----|-------|-------|-----------|--------|-----------------------|
    | N1  | False | False | —         | 99999  | Pure Adam baseline    |
    | N2  | True  | False | 10k       | 99999  | SA effect alone       |
    | N3  | False | True  | —         | 10k    | LBFGS effect alone    |
    | N4  | True  | True  | 10k       | 10k    | SA + LBFGS combined   |

    Comparisons:
    - N1→N2: SA contribution in isolation (Adam only)
    - N1→N3: LBFGS contribution in isolation (no SA)
    - N4 vs (N2, N3): interaction / synergy of SA + LBFGS
    - N4−N3: does SA add anything on top of LBFGS without meta-init?

    Phase 2 (N5–N9): split-point refinement using winner SA setting from Phase 1.
    Created after Phase 1 results — not implemented here.

    Method: grid (4 runs, ~120–150 min each).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_sweepN_AblationSA_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'run_id':      {'values': ["R5"]},
            'sweep_group': {'values': ["N"]},
        },
    }
...
def get_config(sweep_config=None):
    """
    Sweep N — non-meta-init 2x2 factorial.

    Base: ICAD48, no meta-init, 20k iters, SA=False, LBFGS=True.
    All per-run overrides (SA, LBFGS, K_initial, itBFGS) applied by trainer.py
    SWEEP_N routing block. self_adaptive=False in base so SA readback is skipped
    before routing fires (avoids premature K_initial read).

    disable_lr_decay=True: no step decay — routing sets lr_decay_iter=99_999.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260409_SAPINN_sweepN_factorial/"
    config.network_name = "260409_SAPINN_sweepN"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # ==========================================
    # META-INIT — disabled for all runs in Sweep N
    # ==========================================
    config.load_meta_init = False
    config.meta_init_path = ""
    config.warm_start_path = ""   # set per run_id in SWEEP_N routing

    # ==========================================
    # TRAINING PARAMETERS
    # All per-run values overridden by trainer.py SWEEP_N routing
    # ==========================================
    config.training = ml_collections.ConfigDict()
    config.training.iterations = 20_000        # fixed for all N runs
    ...
    # Optimizer — overridden per run_id (use_LBFGS set False for N1/N2)
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 99_999
    config.training.lr_decay_factor = 0.5
    config.training.disable_lr_decay = True
    config.training.use_LBFGS = True          # routing sets False for N1/N2
    config.training.BFGS_lr = 1e-1
    config.training.iterations_before_BFGS = 10_000   # overridden by routing
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — disabled in base; enabled for N2/N4 by routing
    # self_adaptive=False ensures SA readback (line 169) is skipped before
    # routing fires, preventing a premature K_initial read.
    # ==========================================
    config.training.self_adaptive = False      # overridden to True for N2/N4
    config.training.adaptive_sampling = False  # overridden to True for N2/N4
    config.training.tau = 0.02
    config.training.weight_clip = [6, 0.2]
    config.training.beta = 0.2
    config.training.K_initial = 10_000        # overridden by routing for N2/N4
