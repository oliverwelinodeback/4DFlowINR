...
def get_sweep_config():
    """
    SA-PINN LBFGS Multi-Patient Sweep — meta-learned initialization

    Same patients and settings as lbfgs_multipatient_repro.py (repro_SA params,
    itBFGS=10k, lr_decay=8k), but starts from the MAML meta-learned weights
    trained on HV03, HV06, ICAD28, ICAD98, ICAD17, ICAD146.

    Purpose: quantify the benefit of meta-initialization vs random init (repro).
    Direct comparison: run alongside lbfgs_multipatient_repro on same 3 patients.

    Method: grid (3 runs, all train to 40k).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_LBFGS_multipatient_metainit_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-PINN LBFGS multi-patient — meta-learned initialization.

    Identical to lbfgs_multipatient_repro.py except load_meta_init=True, pointing
    to the final checkpoint from Config_260303_SAPINN_maml_newval.py.

    MAML was trained on: HV03, HV06, ICAD28, ICAD98, ICAD17, ICAD146.
    Val cases (these runs): HV01, ICAD48, ICAD21.
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_multipatient_metainit/"
    config.network_name = "260303_lbfgs_multipatient_metainit"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # Optimizer — repro_SA settings: itBFGS=10k, lr_decay=8k
