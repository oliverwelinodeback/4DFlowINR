...
def get_sweep_config():
    """
    SA-PINN LBFGS Multi-Patient Sweep

    Grid sweep over data_file (3 patients). LBFGS settings fixed at best result
    from the ablation sweep: iterations_before_BFGS=10k, lr_decay_iter=8k.

    Patients: HV01 (healthy), ICAD48 (stenosis 50%), ICAD21 (stenosis 70%).
    These are the held-out validation cases for the new MAML run in
    Config_260303_SAPINN_maml_newval.py.

    Method: grid (3 runs, all train to 40k).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_LBFGS_multipatient_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-PINN LBFGS multi-patient sweep — best SA + LBFGS settings fixed.

    SA params from sweep_v2 best run (ICAD48): tau=0.009, beta=0.60, K=20,
    K_initial=5000, weight_clip=[10, 0.1].
    LBFGS fixed at itBFGS=20k (gives SA 15k Adam iters to act before handoff).
    lr_decay disabled — SA handles adaptive weighting; fixed decay confounds results.
    Sweeps over HV01, ICAD48, ICAD21 — the val cases for the new MAML run.

    data_file is a placeholder; overridden per-patient by trainer.py routing.
    """
    ...
    # ==========================================
    # DATA  (placeholder — overridden by sweep routing in trainer.py)
    # ==========================================
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_multipatient_v2/"
    config.network_name = "260303_lbfgs_multipatient"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.constants.venc = 1.7  # HV01 default; overridden by trainer.py routing

    # ==========================================
    # NETWORK ARCHITECTURE
    # omega_0=30: repro_SA setting
    # ==========================================
    ...
    # Optimizer — LBFGS at itBFGS=20k; lr_decay disabled (SA handles weighting)
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 99_999      # disabled: >iterations, no decay
    config.training.lr_decay_factor = 0.5
    config.training.use_LBFGS = True
    config.training.BFGS_lr = 1e-1
    config.training.iterations_before_BFGS = 20_000  # gives SA 15k iters before handoff
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — best params from sweep_v2 (ICAD48)
    # Run: SAPINN_tau0.009_b0.60_K20_Ki5000_wc10-0.1
    # ==========================================
    config.training.self_adaptive = True
    config.training.adaptive_sampling = True
    config.training.tau = 0.009
    config.training.weight_clip = [10, 0.1]
    config.training.beta = 0.60
    ...
    config.predictions.peak_flow_idx = 12  # HV01 default; overridden by trainer.py routing
