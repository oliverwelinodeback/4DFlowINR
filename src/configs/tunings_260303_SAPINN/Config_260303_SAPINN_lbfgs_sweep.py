...
def get_sweep_config():
    """
    SA-PINN LBFGS Ablation Sweep — Option B (lr_decay tied to switch point)

    Grid sweep over iterations_before_BFGS. For each run, Adam's lr is
    decayed 2000 iters before LBFGS takes over (derived in trainer.py readback):
        lr_decay_iter = iterations_before_BFGS - 2000

    This ensures Adam always hands off to LBFGS from a settled, post-decay state,
    eliminating the confounding 25k dip seen when lr_decay_iter is fixed at 25k.

    Values cover the range after SA kicks in (K_initial=10k) up to the last 5k iters:
      10k → lr decay at  8k, LBFGS for last 30k iters
      35k → lr decay at 33k, LBFGS for last  5k iters

    Method: grid (exhaustive — only 6 runs, all train to 40k for fair comparison).

    Requires trainer.py readback block:
        if config.training.use_LBFGS:
            config.training.iterations_before_BFGS = int(sweep_config.get(...))
            config.training.lr_decay_iter = config.training.iterations_before_BFGS - 2_000
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_LBFGS_ablation_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            # Iteration at which Adam is replaced by LBFGS.
            # All values are after K_initial=10k so SA has already fired.
            'training.iterations_before_BFGS': {
                'values': [10_000, 15_000, 20_000, 25_000, 30_000, 35_000],
            },
        },
    }
...
def get_config(sweep_config=None):
    """
    SA-PINN LBFGS Ablation Sweep — HV01 baseline.

    Based on Config_260303_SAPINN_repro_SA.py (confirmed working at chunk_size=10k).
    Option B: lr_decay_iter is derived in trainer.py as iterations_before_BFGS - 2000,
    so Adam is always decayed 2k iters before LBFGS takes over.

    Fixed SA params (repro_SA values — do NOT change for this ablation):
      tau=0.02, beta=0.2, K=20, K_initial=10_000, weight_clip=[6, 0.2]

    Network: omega_0=30 (original paper setting for HV01 repro).
    """
    ...
    # ==========================================
    # DATA  (HV01, LR input — same as repro_SA)
    # ==========================================
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_sweep/"
    config.network_name = "260303_lbfgs_sweep_HV01"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # omega_0=30: original paper setting for HV01 repro (NOT 60)
    # ==========================================
    ...
    config.network.omega_0 = 30         # Original repro_SA setting
    ...
    config.training.lr_decay_iter = 25_000
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — ENABLED (repro_SA params)
    # These are fixed for this ablation — only LBFGS timing is swept.
    # ==========================================
