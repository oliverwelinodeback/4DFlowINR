...
def get_sweep_config():
    """
    SA-PINN LBFGS Multi-Patient Sweep — repro_SA baseline

    Same patients and structure as lbfgs_multipatient_v2, but using the original
    repro_SA hyperparameters (tau=0.02, beta=0.2, K=20, K_initial=10k,
    weight_clip=[6, 0.2]) and original LBFGS settings (itBFGS=10k, lr_decay=8k).

    Purpose: direct comparison against lbfgs_multipatient_v2 (sweep_v2 best params)
    on the same 3 patients.

    Method: grid (3 runs, all train to 40k).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_LBFGS_multipatient_repro_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-PINN LBFGS multi-patient — repro_SA baseline for comparison.

    Uses original SA params from 2510xx sweep (Config_251031_WIRE_MOMENTUM_ALL_SV_SA):
    tau=0.02, beta=0.2, K=20, K_initial=10k, weight_clip=[6, 0.2].
    LBFGS fixed at original ablation best: itBFGS=10k, lr_decay=8k.

    Run alongside Config_260303_SAPINN_lbfgs_multipatient.py (sweep_v2 best params)
    to quantify improvement from the new SA hyperparameters.
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_multipatient_repro/"
    config.network_name = "260303_lbfgs_multipatient_repro"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # Optimizer — original repro_SA LBFGS settings: itBFGS=10k, lr_decay=8k
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 8_000
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — original repro_SA params (from 2510xx sweep)
    # ==========================================
