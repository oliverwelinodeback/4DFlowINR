...
def get_config(sweep_config=None):
    """
    Reproduction of original SA-PINN config — Run A (SA enabled).

    Patient : HV01 (healthy, venc=1.7 m/s)
    Purpose : Exact standalone reproduction of the original sweep config
              (Config_251031_WIRE_MOMENTUM_ALL_SV_SA), locked to HV01.
              Reference point for Run B (noSA) and Run C (HRLR input).

    Key settings preserved from original (do NOT change):
      omega_0=30, out_dim=6, pressure_in_data_loss=False
      use_divergence=False, predict_gradients=True, reference_gradients=True
      use_LBFGS=True but iterations_before_BFGS=99_999 (effectively disabled)
      K_initial=10_000, K=20, tau=0.02, weight_clip=[6,0.2], beta=0.2, chunk_size=2_000

    Overrides vs original sweep config (sweep→standalone):
      sweep=False, data_file fixed to HV01, venc=1.7 (was 1.2 placeholder),
      peak_flow_idx=12 (was 6 placeholder)
    """
    ...
    config.network_name = "260303_repro_SA_HV01"
    ...
    # ==========================================
    # RESOLUTION  (LR input: 0.5 mm × 2 = 1 mm voxels, 20 ms × 2 = 40 ms)
    # ==========================================
    ...
    config.constants.venc = 1.7  # HV01 (sv17); original sweep used 1.2 as placeholder
    ...
    config.network.omega_0 = 30         # Original setting (not 60)
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — ENABLED (original params)
    # ==========================================
    ...
    config.training.pressure_in_data_loss = False  # Original: pressure gradients NOT in data loss
    ...
    config.training.use_divergence = False  # Original: divergence loss disabled
    ...
    config.predictions.peak_flow_idx = 12  # HV01 (meta_config.py); original sweep used 6 placeholder
