...
def get_config(sweep_config=None):
    """
    Reproduction of original SA-PINN config — Run B (SA disabled).

    Patient : HV01 (healthy, venc=1.7 m/s)
    Purpose : Identical to Config_260303_SAPINN_repro_SA.py with SA disabled.
              Isolates the effect of self-adaptive collocation resampling.

    Only changes vs repro_SA:
      self_adaptive=False, adaptive_sampling=False
      network_name = "260303_repro_noSA_HV01"
    """
    ...
    config.network_name = "260303_repro_noSA_HV01"
    ...
    # (all SA params kept identical to repro_SA for reference)
    # ==========================================
