...
def get_sweep_config():
    """
    Iteration-budget ablation — meta-init

    Fixes all settings at repro_SA best + MAML meta-init and sweeps total training
    budget. LBFGS phase is fixed at 6k iters (meta-init converges faster, needing
    less Adam time). trainer.py readback derives iterations_before_BFGS and
    lr_decay_iter automatically from training.iterations and lbfgs_phase_length.
    ...
    return {
        'name': f'SAPINN_metainit_iters_ablation_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'training.iterations': {'values': [15000, 20000, 25000, 30000]},
        },
    }
...
def get_config(sweep_config=None):
    ...
    Identical to lbfgs_repro_iters_ablation except:
    - load_meta_init=True (MAML final checkpoint)
    - lbfgs_phase_length=6_000 (shorter Adam phase sufficient with meta-init)
    - Sweep range: 15k-30k (vs 25k-40k for repro)
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_metainit_iters_ablation/"
    config.network_name = "260303_lbfgs_metainit_iters_ablation"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # ==========================================
    # META-LEARNING (disabled — using pre-trained init only)
    # ==========================================
    ...
    # ==========================================
    # META-INIT — load MAML final checkpoint
    ...
    config.training.lbfgs_phase_length = 6_000    # shorter: meta-init converges by ~5k
    config.training.lr_decay_iter = 22_000         # default (30k - 6k - 2k); overridden by readback
    config.training.lr_decay_factor = 0.5
    config.training.use_LBFGS = True
    config.training.BFGS_lr = 1e-1
    config.training.iterations_before_BFGS = 24_000  # default; overridden by readback
    ...
    # K_initial=6k: matches lbfgs_phase_length so SA fires at the same time Adam
    # starts decaying, giving ~3k Adam iters of SA activity before LBFGS handoff
    # even for the shortest 15k run (itBFGS=9k, SA fires at 6k → 3k overlap).
    # ==========================================
