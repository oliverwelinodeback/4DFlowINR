...
def get_sweep_config():
    """
    Metainit 15k — multi-patient validation

    Validates the metainit 15k result (best from iters ablation on ICAD48) across
    all 3 patients. Settings fixed: iterations=15k, itBFGS=9k, lr_decay=7k,
    K_initial=6k, repro_SA params, meta-init from MAML newval checkpoint.

    Comparison target: lbfgs_multipatient_repro (40k, itBFGS=10k, no meta-init).

    Method: grid (3 runs).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_metainit_15k_multipatient_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    ...
    Best settings from metainit iters ablation (15k run on ICAD48):
    - iterations=15_000, itBFGS=9_000, lr_decay=7_000 (hardcoded, no readback needed)
    - K_initial=6_000: SA fires at 6k, giving 3k Adam iters before LBFGS at 9k
    - repro_SA params (tau=0.02, beta=0.2, K=20, weight_clip=[6,0.2])
    - load_meta_init=True (MAML trained on HV03,HV06,ICAD28,ICAD98,ICAD17,ICAD146)

    data_file overridden per-patient by trainer.py LR_ROUTING.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_metainit_15k_multipatient/"
    config.network_name = "260303_lbfgs_metainit_15k_multipatient"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # ==========================================
    # META-INIT — MAML final checkpoint
    ...
    # ==========================================
    # TRAINING PARAMETERS
    # All values hardcoded — no readback needed for this config
    # ==========================================
    config.training = ml_collections.ConfigDict()
    config.training.iterations = 15_000
    ...
    # Optimizer — fixed at best metainit 15k settings
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 7_000       # 2k before LBFGS at 9k
    config.training.lr_decay_factor = 0.5
    config.training.use_LBFGS = True
    config.training.BFGS_lr = 1e-1
    config.training.iterations_before_BFGS = 9_000
    ...
    # K_initial=6k: SA fires at 6k, giving 3k Adam iters before LBFGS at 9k
    # ==========================================
    ...
    config.training.K_initial = 6_000
