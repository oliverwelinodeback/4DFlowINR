...
def get_sweep_config():
    """
    LBFGS-phase length ablation — repro (random init)

    Fixes total iterations=40k and sweeps iterations_before_BFGS to isolate the
    effect of LBFGS phase length on quality. lr_decay_iter is derived automatically
    in trainer.py readback as itBFGS - 2k.

    Hypothesis: the repro baseline (itBFGS=10k, 30k LBFGS) is good because of the
    long LBFGS phase, not the Adam phase. This sweep tests whether shorter or longer
    LBFGS phases matter.
    ...
    return {
        'name': f'SAPINN_repro_lbfgsphase_ablation_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'training.iterations_before_BFGS': {'values': [5000, 10000, 15000, 20000]},
        },
    }
...
def get_config(sweep_config=None):
    """
    LBFGS-phase length ablation — repro.

    Total iterations fixed at 40k. iterations_before_BFGS swept over [5k,10k,15k,20k].
    trainer.py readback derives lr_decay_iter = itBFGS - 2k automatically when
    iterations_before_BFGS is present in sweep_config.

    10k is the repro baseline setting — included as anchor for direct comparison.
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_lbfgs_repro_lbfgsphase_ablation/"
    config.network_name = "260303_lbfgs_repro_lbfgsphase_ablation"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.training.iterations = 40_000      # fixed across all runs
    ...
    # Optimizer — itBFGS swept; lr_decay derived in trainer.py readback as itBFGS-2k
    config.training.lr = 1e-4
    config.training.lr_decay_iter = 8_000        # default (itBFGS=10k anchor); overridden by readback
    config.training.lr_decay_factor = 0.5
    config.training.use_LBFGS = True
    config.training.BFGS_lr = 1e-1
    config.training.iterations_before_BFGS = 10_000  # anchor; overridden by sweep readback
