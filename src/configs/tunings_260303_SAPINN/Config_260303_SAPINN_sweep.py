...
def get_sweep_config():
    """
    SA-PINN Bayesian Hyperparameter Sweep - Phase 2

    Sweeps the 5 key SA-PINN adaptive collocation parameters:
      - tau:            residual importance threshold (log-uniform scale)
      - beta:           EMA blend factor for weight updates
      - K:              SA update frequency (discrete)
      - K_initial:      warmup iterations before first SA update (discrete)
      - weight_clip_max / weight_clip_min: collocation weight bounds

    Method: Bayesian optimisation (not grid) because the parameter space is too
    large for exhaustive search. Target: 20-30 runs.

    Early termination (hyperband) cuts poor runs at 20k iters, well after the
    SA update fires for the smallest K_initial=1000 setting.

    Requires trainer.py SA readback block to be uncommented (lines ~95-99 + K_initial).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_HyperSweep_{timestamp}',
        'method': 'bayes',
        ...
        'parameters': {
            # Fixed to single patient to avoid confounding patient effects with SA params
            'data_file': {'values': [
                "../data/healthy/HV03_05mm3_20ms_LR_sv13_tSNR10_newMask.h5",
            ]},

            # tau: controls how peaked the residual-to-weight distribution is.
            # Small tau → aggressive upweighting of high-residual points.
            # Log-uniform gives equal probability per decade.
            'training.tau': {
            ...
            # beta: EMA blend factor. Higher = faster adaptation of collocation weights.
            # Linear distribution appropriate for a blend probability.
            'training.beta': {
            ...
            # K: SA update frequency (iterations between refreshes).
            # Discrete because update cost scales with points_to_update/chunk_size.
            'training.K': {
                'values': [10, 25, 50, 100, 250],
            },

            # K_initial: warmup period before first SA update.
            # Must be long enough for residuals to be informative (network past plateau).
            'training.K_initial': {
                'values': [1000, 2500, 5000, 10000],
            },

            # weight_clip_max/min: dynamic range of collocation weight adjustment.
            # Ratio max/min governs how extreme the reweighting can be.
            # Parameterised separately so Bayes can explore the interaction.
            'training.weight_clip_max': {
                'values': [5.0, 10.0, 20.0],
            },
            'training.weight_clip_min': {
...
def get_config(sweep_config=None):
    """
    SA-PINN Bayesian Sweep - Phase 2

    Template: Config_MetaLearning_MAML_PhysicsOuterOnly.py
    (best-known PINN settings: omega_0=60, out_dim=4, grad_weight_scheme, divergence loss)

    SA params are swept via W&B sweep and read back in trainer.py.
    Fixed training settings identical to Phase 1 baseline for fair comparison.
    """
    ...
    config.data_file_ref = "../data/XXX.h5"  # Overridden by trainer data_file routing
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_sweep/"
    config.network_name = "260303_SAPINN_sweep"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.constants.venc = 1.3  # HV03 default; overridden by trainer data routing

    # ==========================================
    # NETWORK ARCHITECTURE
    # (inherited from PhysicsOuterOnly template)
    # ==========================================
    ...
    # ==========================================
    # TRAINING PARAMETERS
    # Fixed at Phase 1 baseline values - only SA params are swept
    # ==========================================
    ...
    # Default values - all overridden by sweep via trainer.py readback block
    # ==========================================
    config.training.self_adaptive = True
    config.training.adaptive_sampling = True
    config.training.tau = 0.01            # Swept: log_uniform [0.005, 0.2]
    config.training.weight_clip = [10, 0.05]  # Swept: max in [5,10,20], min in [0.02,0.05,0.2]
    config.training.beta = 0.3            # Swept: uniform [0.1, 0.7]
    config.training.K_initial = 5_000    # Swept: [1000, 2500, 5000, 10000]
    config.training.K = 250              # Swept: [10, 25, 50, 100, 250]
    ...
    config.training.pressure_in_data_loss = True
