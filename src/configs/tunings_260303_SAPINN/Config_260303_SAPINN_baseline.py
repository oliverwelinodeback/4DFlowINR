...
def get_sweep_config():
    """Dummy sweep config - not used for baseline (sweep=False)."""
    return {
        'name': 'unused_baseline',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-PINN Baseline - Phase 1

    Standalone PINN + Self-Adaptive collocation, no meta-learning.
    ...
    - meta_learning.enabled = False  (no meta-learning)
    - load_meta_init = False          (random init)
    - iterations = 40_000, use_LBFGS = False  (SA + LBFGS untested)
    - self_adaptive = True, adaptive_sampling = True  (SA enabled)
    - SA params: conservative baseline from Config_251031_sweep_WIRE_momentum_SAPINN.py

    Purpose: Establish reproducible SA-PINN baseline before hyperparameter sweeping.
    Monitor in W&B: c_weights/min, c_weights/max, c_weights/p50, c_weights/p90
    to confirm SA update fires after K_initial=5000 and spreads weights.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_baseline/"
    config.network_name = "260303_SAPINN_HV03_baseline"
    ...
    config.coords_characteristic = True  # Must match any fine-tuning config
    ...
    config.constants.venc = 1.3  # HV03 venc

    # ==========================================
    # NETWORK ARCHITECTURE
    # (inherited from PhysicsOuterOnly template - best known PINN settings)
    # ==========================================
    config.network = ml_collections.ConfigDict()
    config.network.in_dim = 4          # [t, x, y, z]
    ...
    config.network.omega_0 = 60        # Best from PhysicsOuterOnly (vs 30 in old SAPINN)
    config.network.complex = False

    # ==========================================
    # META-LEARNING (disabled for standalone PINN)
    # ==========================================
    config.meta_learning = ml_collections.ConfigDict()
    config.meta_learning.enabled = False

    # Required fields (trainer reads these even when enabled=False)
    ...
    # ==========================================
    # META-INIT (disabled - random initialization)
    # ==========================================
    ...
    # LBFGS disabled: SA-PINN + LBFGS interaction untested
    ...
    # Conservative baseline values from Config_251031_sweep_WIRE_momentum_SAPINN.py
    # ==========================================
    config.training.self_adaptive = True
    config.training.adaptive_sampling = True
    config.training.tau = 0.01            # Residual importance threshold
    config.training.weight_clip = [10, 0.05]  # [max, min] collocation weight bounds
    config.training.beta = 0.3            # EMA blend factor for weight updates
    config.training.K_initial = 5_000    # Iters before first SA update
    config.training.K = 250              # SA update frequency after K_initial
    ...
    config.training.pressure_in_data_loss = True   # corrected: gradients available in out_dim=6
    ...
    config.training.use_divergence = True   # Inherited from PhysicsOuterOnly template
    ...
    config.predictions.peak_flow_idx = 14  # HV03
