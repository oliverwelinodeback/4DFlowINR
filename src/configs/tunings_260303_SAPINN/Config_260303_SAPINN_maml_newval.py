...
def get_sweep_config():
    ...
    return {
        'name': 'unused_SAPINN_maml_newval',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    MAML (second-order) with Physics in Outer Loop Only — new validation split.

    Identical to Config_260303_SAPINN_maml.py except train/val cases are reshuffled:
      - Validation: HV01, ICAD48, ICAD21 (the 3 patients used in lbfgs_multipatient sweep)
      - Training: HV03, HV06, ICAD28, ICAD98, ICAD17, ICAD146 (remaining 6)

    This produces a meta-init that has never seen HV01/ICAD48/ICAD21, so the
    meta-weights are a fair prior for evaluating those patients.

    Network: out_dim=6, omega_0=30, predict_gradients=True — matches lbfgs_multipatient.
    """
    ...
    # ==========================================
    # DATA  (HV01 — validation patient, used for fine-tune eval after meta)
    # ==========================================
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_maml_newval/"
    config.network_name = "260303_MAML_SAPINN_HV01_ICAD48_ICAD21_val"
    ...
    # Must match lbfgs_multipatient fine-tuning config exactly.
    # ==========================================
    ...
    config.constants.venc = 1.7  # HV01; overridden per case by case_venc

    # ==========================================
    # NETWORK ARCHITECTURE
    # out_dim=6 and omega_0=30 must match lbfgs_multipatient config.
    # ==========================================
    ...
    config.network.omega_0 = 30         # Must match lbfgs_multipatient
    config.network.complex = False

    # ==========================================
    # META-LEARNING CONFIGURATION
    # Identical to Config_260303_SAPINN_maml.py (gold standard for out_dim=6)
    # ==========================================
    ...
    # Training cases: original 6 minus HV01/ICAD48/ICAD21, replaced by HV06/ICAD98/ICAD146
    ...
    # Validation cases: the 3 patients used in the lbfgs_multipatient sweep
    ...
    # Matches lbfgs_multipatient settings.
    # ==========================================
