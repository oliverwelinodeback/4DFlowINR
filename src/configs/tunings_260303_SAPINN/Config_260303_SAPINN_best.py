...
def get_config(sweep_config=None):
    """
    SA-PINN with Phase 2 best hyperparameters (Step 2).
    ...
    Purpose : Direct comparison against Config_260303_SAPINN_noSA_baseline.py.
              Tests whether SA-PINN with the best found hyperparameters improves
              over standard PINN on the corrected network configuration.

    !! FILL IN Phase 2 best SA hyperparameters before running:
       config.training.tau        (was swept: log_uniform [0.005, 0.2])
       config.training.beta       (was swept: uniform [0.1, 0.7])
       config.training.K          (was swept: [10, 25, 50, 100, 250])
       config.training.K_initial  (was swept: [1000, 2500, 5000, 10000])
       config.training.weight_clip (was swept: max [5,10,20], min [0.02,0.05,0.2])
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_best/"
    config.network_name = "260303_SAPINN_ICAD98_best"
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — PHASE 2 BEST PARAMS
    # ==========================================
    config.training.self_adaptive = True
    config.training.adaptive_sampling = True
    config.training.tau = 0.106         
    config.training.weight_clip = [20, 0.05]  
    config.training.beta = 0.157           
    config.training.K_initial = 5_000    
    config.training.K = 100           
