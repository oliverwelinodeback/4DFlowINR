...
def get_sweep_config():
    """
    SA-PINN Hyperparameter Sweep v2 — correct settings.

    Bayesian sweep over SA params using the current best model configuration:
      omega_0=30, out_dim=6, use_divergence=False, pressure_in_data_loss=False

    This is a re-run of the Phase 2 sweep (Config_260303_SAPINN_sweep.py) which was
    never actually executed. The original sweep config used wrong settings
    (omega_0=60, use_divergence=True, pressure_in_data_loss=True) that are not
    comparable to the current best config.

    The repro_SA params (tau=0.02, beta=0.2, K=20, K_initial=10k, weight_clip=[6,0.2])
    used in all current experiments came from the older 2510xx sweep, not from a sweep
    on this model variant. This sweep validates or improves on those params.

    LBFGS: fixed at iterations_before_BFGS=20_000, lr_decay_iter=18_000.
    This gives SA 10k iterations (500 refresh cycles at K=20) to reshape collocation
    weights under Adam before LBFGS takes over.

    Patient: ICAD48 (stenosis 50%, one of the lbfgs_multipatient val cases).
    Method: Bayesian, 20-30 runs, early_terminate at 20k iters (after K_initial fires).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_sweep_v2_{timestamp}',
        'method': 'bayes',
        ...
        'parameters': {
            # Temperature of the SA weight update
            'training.tau': {
                'distribution': 'log_uniform_values',
                'min': 0.005,
                'max': 0.2,
            },
            # Exponential moving average factor for weight update
            'training.beta': {
                'distribution': 'uniform',
                'min': 0.1,
                'max': 0.7,
            },
            # SA refresh frequency (every K iters)
            'training.K': {
                'values': [10, 20, 50, 100],
            },
            # Iteration at which SA first fires
            'training.K_initial': {
                'values': [5000, 10000, 15000],
            },
            # Upper clip for SA weights
            'training.weight_clip_max': {
                'values': [5.0, 6.0, 10.0],
            },
            # Lower clip for SA weights
...
def get_config(sweep_config=None):
    """
    SA-PINN Hyperparameter Sweep v2 — HV01 baseline.

    Based on Config_260303_SAPINN_repro_SA.py (confirmed working at chunk_size=10k).
    Only differences from repro_SA:
      - sweep=True, patient changed to ICAD48 (stenosis 50%, sv13, venc=1.3)
      - LBFGS enabled with fixed itBFGS=20k, lr_decay=18k (SA gets 10k iters to settle)
      - networks_folder → 260303_SAPINN_sweep_v2

    SA params (tau, beta, K, K_initial, weight_clip) are overridden by sweep readback
    in trainer.py lines ~167-175. Current best values are kept as defaults / anchors:
      tau=0.02, beta=0.2, K=20, K_initial=10_000, weight_clip=[6, 0.2]
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_sweep_v2/"
    config.network_name = "260303_SAPINN_sweep_v2_ICAD48"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.constants.venc = 1.3  # ICAD48 (sv13)

    # ==========================================
    # NETWORK ARCHITECTURE
    # omega_0=30: current best setting (NOT 60)
    # out_dim=6: matches lbfgs_multipatient
    # ==========================================
    ...
    # Optimizer — LBFGS fixed at itBFGS=20k, giving SA 10k iters to settle first.
    # lr_decay disabled (99_999 > iterations): SA handles adaptive weighting; a fixed
    # lr decay would confound SA param results by interacting with K_initial timing.
    ...
    config.training.iterations_before_BFGS = 20_000
    ...
    # Default values = repro_SA anchor params; overridden by sweep readback.
    # ==========================================
    ...
    # predict_gradients=True: matches out_dim=6
    # ==========================================
