...
def get_sweep_config():
    """
    SA-PINN HRLR Sweep — all 9 patients × tSNR10 + tSNR2

    Grid sweep over data_file only (18 runs). All other settings are fixed at
    the repro_HRLR baseline (HV01, tSNR10). Per-patient ref file, venc, and
    peak_flow_idx are resolved in trainer_hrlr.py via the HRLR_ROUTING dict.

    Purpose: compare SA-PINN performance across patients and noise levels using
    HRLR input (same spatial resolution as reference, no 2x downsampling).

    Method: grid (exhaustive — all 18 runs train to 40k for fair comparison).
    Run with trainer_hrlr.py (not trainer.py).
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_HRLR_allpatients_{timestamp}',
        'method': 'grid',
        ...
        'parameters': {
            'data_file': {'values': [
                ...
                "../data/healthy/HV03_05mm3_20ms_HRLR_sv13_tSNR10.h5",
                "../data/healthy/HV06_05mm3_20ms_HRLR_sv12_tSNR10.h5",
                "../data/stenosis_50/ICAD28_05mm3_20ms_HRLR_sv13_tSNR10.h5",
                "../data/stenosis_50/ICAD48_05mm3_20ms_HRLR_sv13_tSNR10.h5",
                "../data/stenosis_50/ICAD98_05mm3_20ms_HRLR_sv51_tSNR10.h5",
                "../data/stenosis_70/ICAD17_05mm3_20ms_HRLR_sv41_tSNR10.h5",
                "../data/stenosis_70/ICAD21_05mm3_20ms_HRLR_sv26_tSNR10.h5",
                "../data/stenosis_70/ICAD146_05mm3_20ms_HRLR_sv17_tSNR10.h5",
                # --- tSNR2 ---
                "../data/healthy/HV01_05mm3_20ms_HRLR_sv17_tSNR2.h5",
                "../data/healthy/HV03_05mm3_20ms_HRLR_sv13_tSNR2.h5",
                "../data/healthy/HV06_05mm3_20ms_HRLR_sv12_tSNR2.h5",
                "../data/stenosis_50/ICAD28_05mm3_20ms_HRLR_sv13_tSNR2.h5",
                "../data/stenosis_50/ICAD48_05mm3_20ms_HRLR_sv13_tSNR2.h5",
                "../data/stenosis_50/ICAD98_05mm3_20ms_HRLR_sv51_tSNR2.h5",
                "../data/stenosis_70/ICAD17_05mm3_20ms_HRLR_sv41_tSNR2.h5",
                "../data/stenosis_70/ICAD21_05mm3_20ms_HRLR_sv26_tSNR2.h5",
                "../data/stenosis_70/ICAD146_05mm3_20ms_HRLR_sv17_tSNR2.h5",
            ]},
        },
    }
...
def get_config(sweep_config=None):
    """
    SA-PINN HRLR Sweep — based on Config_260303_SAPINN_repro_HRLR.py.

    HRLR-specific settings (must NOT be changed):
      ref_spatial_factor = 1  — HRLR is already at reference resolution
      resolution.dx/dy/dz = 0.0005  — 0.5 mm native HR voxels
      resolution.dt = 0.02          — 20 ms native
      plot.spatial_factor = 1
      plot.temporal_factor = 1

    data_file / data_file_ref / venc / peak_flow_idx are overridden per run
    by trainer_hrlr.py using the HRLR_ROUTING lookup dict.

    SA params fixed at repro_HRLR values (do not change for this sweep):
    ...
    # ==========================================
    # DATA  (placeholders — overridden by trainer_hrlr.py routing)
    # ==========================================
    ...
    config.ref_spatial_factor = 1   # HRLR: no upsampling needed
    config.ref_temporal_factor = 1

    # ==========================================
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260303_SAPINN_hrlr_sweep/"
    config.network_name = "260303_hrlr_sweep"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.resolution.dx = 0.0005   # ×1 — native HR, no factor-2 scaling
    ...
    config.constants.venc = 1.7  # HV01 default; overridden per run by trainer_hrlr.py
    ...
    # ==========================================
    # SELF-ADAPTIVE PINN — ENABLED (repro_HRLR params)
    # ==========================================
    ...
    config.predictions.peak_flow_idx = 12   # HV01 default; overridden per run
