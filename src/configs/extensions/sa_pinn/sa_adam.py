...
def get_sweep_config():
    """
    SA-PINN replication — SA-Adam-only optimizer.

    Sweeps over 3 patients × 3 data types (LR, HRLR-tSNR10, HRLR-tSNR2).
    Optimizer: 30k Adam with self-adaptive weights, no L-BFGS.

    Corresponds to R3 runs from the original factorial experiment.
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'SAPINN_SA_Adam_{timestamp}',
        'method': 'grid',
...
def get_config(sweep_config=None):
    """
    SA-Adam-only replication of the factorial experiment (R3 runs).
    30k Adam with self-adaptive weights, L-BFGS disabled.
