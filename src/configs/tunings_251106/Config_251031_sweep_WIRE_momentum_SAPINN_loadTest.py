...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE_MOMENTUM_SA2_{timestamp}', 
        'method': 'grid',
        ...
        'parameters': {
            ...
            'data_file': {'values': [
                ...
                #"../data/healthy/HV01_05mm3_20ms_LR_dv_hv17_tSNR8.h5",   ### CTUE HERE
...
def get_config(sweep_config=None):
    ...
    # Model 
    config.networks_folder = "../models/251106_SAPINN"
    config.network_name = "251031_WIRE_SAPINN_Aggressive" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.training.iterations = 160 #80 #15000 #!#
    config.training.data_points_per_batch = 5000 # None to use all #20000
    config.training.coll_points_per_batch = 5000 # None to use all #20000
    ...
    config.training.iterations_before_BFGS = 99_500 #!#
    # Loss details
    ...
    config.training.tau = 0.01 # 0.03
    config.training.weight_clip = [10, 0.05] # [5, 0.2]
    config.training.beta = 0.3 # 0.2
    config.training.K_initial = 5_000 #10_000 # 500
    config.training.K = 250 # 250
    ...
    config.training.summary_iter = 40
