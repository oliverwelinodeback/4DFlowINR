...
def get_config(sweep_config=None):
    ...
    # Model
    config.networks_folder = "../models/MetaLearning_FOMAML_DataDriven/"
    config.network_name = "260202_FOMAML_DataDriven"
    ...
    config.meta_learning.meta_method = 'FOMAML'  # First-order MAML (memory efficient)

    # Legacy flag (for backward compatibility)
    config.meta_learning.use_first_order = True  # True = FOMAML (first-order)

    # Reptile-specific settings (not used for FOMAML, but kept for consistency)
    ...
    config.meta_learning.meta_batch_size = 4  # Can be higher than MAML due to memory savings
