...
def get_sweep_config():
    """
    Sweep configuration for Meta-PINN (FOMAML with Physics Loss).

    Key differences from data-driven:
    - Physics loss enabled in inner/outer loops
    - Sweep includes physics_weight and coll_points_inner
    - Lower meta_batch_size due to physics computation memory
    - More inner steps (literature recommends 10-20 for PINNs)
    """
    return {
        'name': 'Meta_FOMAML_PINN',
        'method': 'bayes',
        ...
        'parameters': {
            ...
            'meta_learning.inner_steps': {
                'values': [5, 7, 10]  # More steps for physics-informed
            },
            'meta_learning.meta_batch_size': {
                'values': [2, 3]  # Lower due to physics memory overhead
            },
            ...
            'meta_learning.coll_points_inner': {
...
def get_config(sweep_config=None):
    ...
    """
    Meta-PINN Configuration (FOMAML with Navier-Stokes Physics Loss)

    This config enables physics-informed meta-learning where the inner loop
    optimizes both data loss AND Navier-Stokes PDE residual.

    Key settings:
    - meta_method: 'FOMAML' (first-order for memory efficiency with physics)
    - use_physics_loss: True (Navier-Stokes in inner/outer loops)
    - out_dim: 4 (u, v, w, p for pressure gradient computation)
    """
    ...
    # Model
    config.networks_folder = "../models/MetaLearning_FOMAML_PINN/"
    config.network_name = "260203_FOMAML_PINN"
    ...
    config.network.out_dim = 4  # PINN: u, v, w, p (pressure needed for Navier-Stokes)
    ...
    # ==========================================
    # META-LEARNING CONFIGURATION (PINN)
    # ==========================================
    ...
    config.meta_learning.meta_method = 'FOMAML'  # First-order for memory efficiency

    # Legacy flag (use meta_method instead)
    config.meta_learning.use_first_order = True

    # Reptile-specific (not used for FOMAML, but kept for consistency)
    config.meta_learning.reptile_epsilon = 1.0

    # Inner loop settings (SGD optimizer)
    config.meta_learning.inner_lr = 0.001
    config.meta_learning.inner_steps = 7  # More steps for physics-informed (literature: 10-20)
    config.meta_learning.inner_points = 5000       # Not used - uses ALL support points
    config.meta_learning.coll_points_inner = 2000  # Collocation points for physics loss
    config.meta_learning.coll_points_outer = 2000  # Collocation points for outer loop
    config.meta_learning.boundary_points_inner = 2000

    # Outer loop settings (Adam optimizer)
    config.meta_learning.outer_lr = 0.0001
    config.meta_learning.meta_batch_size = 2  # Lower due to physics memory overhead
    ...
    config.meta_learning.use_physics_loss = True   # ENABLED: Navier-Stokes in inner/outer loops
    config.meta_learning.physics_weight = 1.0      # Weight for physics loss (sweep this!)
    config.meta_learning.use_boundary_loss = False # Boundary loss (optional)
    config.meta_learning.div_weight = 1.0          # Divergence weight multiplier

    # Gradient-based loss weight balancing
    config.meta_learning.use_grad_weights = False

    # Physical Pre-Conditioning (Phase 0) - optional warm-start
