...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE_SV_MetaLearn_{timestamp}', 
        'method': 'grid',
...
""" def get_sweep_config():
    return {
        'name': 'Meta_SR_Hyperparameter_Tuning',
        'method': 'bayes',  # Bayesian optimization is faster than grid for finding "sweet spots"
        'metric': {
            'name': 'Val/Post_Adapt_HR_Loss', 
            'goal': 'minimize'
        },
        'parameters': {
            
            # --- The Most Critical Parameter ---
            # You found success with ~0.01. Let's explore around it.
            ...
            # --- Adaptation Depth ---
            # More steps = better adaptation, but harder to train (vanishing gradients)
            'meta_learning.inner_steps': {
                'values': [3, 5, 10]
            },

            # --- Stability Control ---
            # If gradients explode, lower this.
            ...
            # --- Noise Reduction ---
            # Your training loss is very spiky. Increasing batch size helps stability.
            # (Be careful with VRAM!)
...
def get_config(sweep_config=None):
    ...
    # Model 
    config.networks_folder = "../models/251218_WIRE_MetaLearn/"
    config.network_name = "251217_WIRE_MetaLearn" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.meta_learning.inner_lr = 0.0011597501730668334
    config.meta_learning.inner_steps = 5
    config.meta_learning.outer_lr = 1.2271247350707506e-05
    ...
    # NTK Analysis
    config.ntk = ml_collections.ConfigDict()
    config.ntk.enabled = True
    config.ntk.num_points = 10_000
    config.ntk.num_eig = 50
    config.ntk.batch_size = 128
    config.ntk.save_dir = "figures/ntk_analysis"

    # Fine-tuning
    config.load_meta_init = True
    config.meta_init_path = "../models/finetune_ICAD146_meta/251218_WIRE_SV_NewMask_MetaLearn_innerLR0.0011597501730668334_innerSteps5_outerLR1.2271247350707506e-05_metaBatch4_20251218-1339/meta_learned_init_FINAL.pth"
    
    # Update save paths
    config.networks_folder = "../models/finetune_ICAD146_meta/"
    config.network_name = "finetune_ICAD146_from_meta"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    config.log_dir = f"{config.networks_folder}/{config.network_name}_{timestamp}"
