...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE_SV_MAML_PINN_Scheduler_{timestamp}', 
        'method': 'grid',
...
""" def get_sweep_config():
    return {
        'name': 'Meta_MAML_PINN_DataWeigths',
        'method': 'bayes',
...
def get_config(sweep_config=None):
    ...
    # Model
    config.networks_folder = "../models/MetaLearning_BestHyperParameters_PINN/"
    config.network_name = "260202_MAML_PINN_DataWeigths"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.network.out_dim = 4  # Data-driven: velocity only (u, v, w)
    ...
    config.meta_learning.use_first_order = False  # False = full MAML (second-order)

    # Reptile-specific settings (not used for MAML, but kept for consistency)
    ...
    config.meta_learning.coll_points_inner = 3000  # Not used for data-driven
    ...
    config.meta_init_path = "../models/MetaLearning_BestHyperPArameters/MetaMAML_PINN_innerLR0.0006110407514672489_innerSteps10_outerLR1.5100213760956933e-05_metaBS4_20260203-1749/meta_best.pth"
