...
def get_sweep_config():
    """
    Sweep configuration for fine-tuning after Data-Driven MAML meta-learning.

    This config is for PINN fine-tuning of a model that was meta-learned
    with data-only loss but outputs 4 channels (u, v, w, p).

    The meta-learned weights provide good initialization for velocities.
    Fine-tuning with physics loss teaches the model to predict pressure.

    Key questions to answer:
    1. Does meta-init help vs random init?
    2. How many iterations needed with meta-init?
    3. Is LBFGS helpful or harmful with meta-init?
    4. What learning rate works best?
    5. How much physics weight?
    """
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'MAML_DataDriven_ForPINN_10it_{timestamp}',
        'method': 'grid',
        ...
        'parameters': {
            ...
            # Compare meta-init vs random
            'load_meta_init': {'values': [True, False]},
            # Test different training lengths
            #'training.iterations': {'values': [5000, 8000, 12000]},
            # Test with/without LBFGS
            #'training.use_LBFGS': {'values': [True, False]},
            # Learning rate (lower for meta-init)
            #'training.lr': {'values': [1e-4, 5e-5]},
        },
    }
...
""" def get_sweep_config_quick():
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'MAML_FineTune_Quick_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'data_file': {'values': [
            ...
            'load_meta_init': {'values': [True, False]},
            'training.iterations': {'values': [5000]},
            'training.use_LBFGS': {'values': [False]},  # Start without LBFGS
        },
    } """


""" def get_sweep_config_lbfgs():
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    return {
        'name': f'MAML_FineTune_LBFGS_{timestamp}',
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'data_file': {'values': [
            ...
            'load_meta_init': {'values': [True]},  # Only meta-init
            'training.iterations': {'values': [8000, 12000]},
            'training.use_LBFGS': {'values': [True]},
            'training.iterations_before_BFGS': {'values': [5000, 7000, 10000]},
            'training.BFGS_max_iter': {'values': [2, 3, 5]},
            'training.lr': {'values': [5e-5, 1e-4]},
        },
    } """
...
def get_config(sweep_config=None):
    """
    Data-Driven MAML with 4-channel output for PINN fine-tuning.

    Strategy:
    1. Meta-learn with DATA LOSS ONLY (velocities u, v, w)
    2. Model outputs 4 channels (u, v, w, p) - pressure channel untrained
    3. Fine-tune with PINN loss - physics teaches pressure

    This approach:
    - Uses proven data-driven MAML (works well)
    - Prepares architecture for physics (out_dim=4)
    - Pressure learned purely from Navier-Stokes during fine-tuning
    """
    ...
    # Model
    config.networks_folder = "../models/MetaLearning_MAML_DataDriven_ForPINN/"
    config.network_name = "260204_MAML_DataDriven_ForPINN"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.setup.include_pressure = True  # IMPORTANT: Enable pressure for PINN
    ...
    config.network.out_dim = 4  # IMPORTANT: u, v, w, p (pressure for Navier-Stokes)
    ...
    # ==========================================
    # META-LEARNING CONFIGURATION (Data-Driven)
    # ==========================================
    config.meta_learning = ml_collections.ConfigDict()
    config.meta_learning.enabled = False  # Set to True for meta-learning phase
    ...
    # Inner loop settings - BEST HYPERPARAMETERS from data-driven sweep
    config.meta_learning.inner_lr = 0.000611
    config.meta_learning.inner_steps = 10
    config.meta_learning.inner_points = 5000
    config.meta_learning.coll_points_inner = 3000  # Not used (data-driven)
    ...
    # ==========================================
    # DATA-DRIVEN META-LEARNING (NO PHYSICS)
    # ==========================================
    # Meta-learning uses DATA LOSS ONLY
    # Physics will be added during fine-tuning
    config.meta_learning.support_fraction = 0.5
    config.meta_learning.use_physics_loss = False  # DATA-DRIVEN: No physics
    config.meta_learning.use_physics_outer_only = False
    ...
    # Physical Pre-Conditioning - disabled for data-driven
    ...
    # ==========================================
    # FINE-TUNING WITH PINN
    # ==========================================
    # After meta-learning, load weights and fine-tune with physics
    config.load_meta_init = True
    config.meta_init_path = "../models/MetaLearning_MAML_DataDriven_ForPINN/260204_MAML_DataDriven_ForPINN_20260204-1046/meta_best.pth"  # UPDATE THIS

    # Training parameters for PINN fine-tuning
    ...
    # Scheduler
    # NOTE: 'exp' scheduler decays LR too aggressively when scheduler.step() is called every iteration
    # Set to 'none' to match trainer_wandb_SA.py behavior (which disables scheduler)
    ...
    config.training.pressure_in_data_loss = False  # No pressure ground truth
    ...
    # ==========================================
    # PHYSICS LOSS FOR FINE-TUNING (ENABLED)
    # ==========================================
    # This is where pressure is learned from Navier-Stokes
    ...
    config.training.physics_weight = 1.0  # Tune this if needed
