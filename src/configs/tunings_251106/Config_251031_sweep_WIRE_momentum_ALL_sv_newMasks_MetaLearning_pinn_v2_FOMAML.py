...
""" def get_sweep_config():
    ...
    return {
        'name': f'WIRE_SV_MetaLearn_PINN_{timestamp}', 
        'method': 'grid',
...
def get_config(sweep_config=None):
    ...
    # Model
    config.networks_folder = "../models/MetaLearningPINNSweepv4/"
    config.network_name = "260130_MetaLearn_Reptile"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.meta_learning.meta_method = 'Reptile'  # Options: 'MAML', 'FOMAML', 'Reptile'

    # Legacy flag (use meta_method instead)
    config.meta_learning.use_first_order = True  # True for FOMAML, False for MAML

    # Reptile-specific settings
    config.meta_learning.reptile_epsilon = 1.0  # Interpolation factor for Reptile

    # Inner loop settings (SGD optimizer)
    config.meta_learning.inner_lr = 0.0002
    config.meta_learning.inner_steps = 5
    config.meta_learning.inner_points = 5000       # Max data points per inner step
    config.meta_learning.coll_points_inner = 3000  # Collocation points for inner loop
    config.meta_learning.boundary_points_inner = 2000  # Boundary points for inner loop

    # Outer loop settings (Adam optimizer)
    config.meta_learning.outer_lr = 0.001
    ...
    # Loss settings (full PINN inner loop)
    config.meta_learning.support_fraction = 0.5    # Split LR: 50% support, 50% query
    config.meta_learning.use_physics_loss = False   # Enable Navier-Stokes in inner loop
    config.meta_learning.physics_weight = 1.0      # Weight for physics loss
    config.meta_learning.use_boundary_loss = False # Enable boundary loss in inner loop
    config.meta_learning.div_weight = 1.0          # Divergence weight multiplier

    # Gradient-based loss weight balancing (detached - works with FOMAML)
    config.meta_learning.use_grad_weights = False  # Enable gradient-based loss balancing

    # Physical Pre-Conditioning (Phase 0)
    config.meta_learning.use_physics_preconditioning = False  # Enable warm-start with physics
    config.meta_learning.preconditioning_iters = 100          # Iterations for Phase 0
    config.meta_learning.preconditioning_cases = 5            # Number of sample cases
    config.meta_learning.preconditioning_lr = 1e-3            # Learning rate for pre-conditioning
