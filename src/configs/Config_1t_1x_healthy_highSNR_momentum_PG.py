...
def get_config(sweep_config=None):
    ...
    config.sweep = False
    # Data
    config.data_file = "../../data/data_05mm_incl_pressure/healthy-05mm3_dv_highSNR_x1.h5" ## Insert correct file here
    config.include_ref = True
    config.include_ref_loss = True
    config.data_file_ref = "../../data/data_05mm_incl_pressure/healthy-05mm3.h5" ## Insert correct file here
    config.ref_spatial_factor = 1
    config.ref_temporal_factor = 1

    # Model 
    networks_folder = "../models/250422_PINN_PG_test" ## Change this
    config.network_name = "Config_1t_1x_HV01_highSNR_PINN_SIREN_sweep" 
    ...
    config.collocation_points = 800_000
    ...
    config.training.iterations_before_BFGS = 99000
    # Loss details
    ...
    config.training.reference_gradients = False ## change this
    config.training.use_divergence = False
    config.training.physics_weight = 10
    ...
    config.predictions.peak_flow_idx = 25
