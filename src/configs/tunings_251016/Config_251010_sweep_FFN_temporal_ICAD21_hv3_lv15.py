...
def get_sweep_config():
    ...
    return {
        'name': f'FFN_TEMPORAL_bias_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'template.t_len': {'values': [10, 100, 1000, 10_000, 100_000, 1_000_000, 10_000_000]},
        },
    }
...
def get_config():
    ...
    # Data
    config.data_file = "../data/stenosis_70/ICAD21_05mm3_20ms_LR_dv_tSNR8_hv3_lv15.h5"
    ...
    # Model 
    config.networks_folder = "../models/251016_ICAD21_FFN_sweep_TEMPORAL2/"
    config.network_name = "ICAD21_hv3_FFN_TEMPORAL" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.domain.t_end = -(-37 // 2) # Ceiling division
    config.domain.x_start = int(0/2)
    config.domain.x_end = int(120/2)
    config.domain.y_start = int(0/2)
    config.domain.y_end = int(80/2)
    config.domain.z_start = int(0/2)
    config.domain.z_end = int(64/2)
    ...
    config.network.fourier_mapping_size = 256 ##### CHANGE HERE #####
    config.network.fourier_scale = 4.5 ##### CHANGE HERE #####
    
    # WIRE parameters
    #config.network.complex = True
    #config.network.omega_0 = 10
    ...
    config.predictions.flow_idx2 = 32
