...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE_loss_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'training.loss': {'values': ['cos', 'mse', 'mse_and_cos']},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251016_ICAD21_WIRE_LOSS_sweep/"
    config.network_name = "WIRE_ICAD21_LOSS" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
