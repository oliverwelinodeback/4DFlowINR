...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-REAL_training_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'training.iterations_before_BFGS': {'values': [9999, 6000]},
            'training.lr_decay_iter': {'values': [9999, 2000]},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251016_WIRE_REAL_training_sweep/"
    ...
    config.training.lr_decay_iter = 4000
