...
def get_sweep_config():
    ...
    return {
        ...
        'parameters': {
            'training.iterations': {'values': [150_000, 15_000]},
        },
    }
...
def get_config(sweep_config=None):
    ...
    # Model 
    config.networks_folder = "../models/251016_NS_REY_tuning/"
    config.network_name = "HV01_WIRE_momentum" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.training.iterations_before_BFGS = 145_000 #!#
    # Loss details
    ...
    config.training.summary_iter = 15000
    config.training.log_iter = 1000
    config.training.error_iter = 15000
    config.training.denormalize = True
    # Plotting
    config.plot = ml_collections.ConfigDict()
    config.plot.iter = 15000
    config.plot.gt = True
