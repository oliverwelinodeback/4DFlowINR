...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-REAL_training_sweep3_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'template.t_len': {'values': [500]},
            'training.sample_boundary': {'values': [False, True]},
            'network.sigma_0': {'values': [0]},
            'training.sample_collocation': {'values': [False, True]},
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251016_WIRE_REAL_training_sweep4/"
    config.network_name = "HV01_WIRE_REAL" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
