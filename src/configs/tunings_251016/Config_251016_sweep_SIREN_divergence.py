...
def get_sweep_config():
    ...
    return {
        'name': f'SIREN_divergence_boundary_sweep_{timestamp}', 
        'method': 'grid',
        ...
        'parameters': {
            ...
            'training.sample_boundary': {'values': [False, True]},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251016_SIREN_divergence_boundary_loss_sweep/"
    config.network_name = "HV01_SIREN" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    config.training.physics_loss_on_data_points = config.training.use_physics_loss
