...
def get_sweep_config():
    ...
    return {
        'name': f'SIREN_bias_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'Loss/Ref', 'goal': 'minimize'},
        'parameters': {
            'network.omega_0': {'values': [1, 10, 30, 70]},
        },
    }
...
def get_config():
    ...
    # Model 
    networks_folder = "../models/251010_sweeps/"
    config.network_name = "HV01_WIRE_MSE_SIREN_bias_sweep_test" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
