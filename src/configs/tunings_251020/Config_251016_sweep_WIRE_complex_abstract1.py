...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-COMPLEX_abstract1_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'data_file': {'values': ["../data/healthy/HV01_05mm3_20ms_LR_dv_hv17_tSNR8.h5", "../data/healthy/HV01_05mm3_20ms_sv08_tSNR10.h5"]},
            'training.loss': {'values': ['cos', 'mse']},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251020_WIRE_CMPLX_abstract1/"
    config.network_name = "HV01_WIRE_CMPLX" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
