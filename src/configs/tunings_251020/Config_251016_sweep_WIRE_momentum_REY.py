...
def get_sweep_config():
    ...
    return {
        'name': f'NS_Tunings_REY_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': "FINAL Pressure Gradient Relative Error [Fluid]", 'goal': 'minimize'},
        'parameters': {
            'constants.U': {'values': [1.0, 4.0, 2.0, 0.5]},
            'constants.L': {'values': [0.005, 0.1, 0.01, 0.0005]},
        },
    }
...
def get_config(sweep_config=None):
    ...
    # Model 
    config.networks_folder = "../models/251020_NS_REY_tuning/"
