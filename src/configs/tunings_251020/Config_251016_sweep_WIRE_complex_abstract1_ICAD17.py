...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-COMPLEX_abstract1_ICAD17_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'data_file': {'values': ["../data/stenosis_70/ICAD17_05mm3_20ms_LR_dv_hv41_tSNR8.h5"]},
            'constants.U': {'values': [1.0, 4.0]},
        },
    }
...
def get_config():
    ...
    # Data
    config.data_file = "../data/stenosis_70/ICAD17_05mm3_20ms_LR_dv_hv41_tSNR8.h5" #### CTNUE ERHEHREHRHEHRE
    ...
    # Model 
    config.networks_folder = "../models/251020_WIRE_CMPLX_abstract1_ICAD17/"
    config.network_name = "ICAD17_WIRE_CMPLX" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
    ...
    # config.domain.t_start = 0
    # config.domain.t_end = -(-35 // 2) # Ceiling division
    # config.domain.x_start = int(0/2)
    # config.domain.x_end = int(190/2)
    # config.domain.y_start = int(0/2)
    # config.domain.y_end = int(110/2)
    # config.domain.z_start = int(0/2)
    # config.domain.z_end = int(60/2)
