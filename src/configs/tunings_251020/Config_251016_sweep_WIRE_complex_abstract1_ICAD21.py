...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-COMPLEX_abstract1_ICAD_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'data_file': {'values': ["../data/stenosis_70/ICAD21_05mm3_20ms_LR_dv_hv26_tSNR8.h5", "../data/stenosis_70/ICAD21_05mm3_20ms_LR_dv_tSNR8.h5"]},
            'training.loss': {'values': ['cos', 'mse', 'cos_and_mse']},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251020_WIRE_CMPLX_abstract1_ICAD/"
    config.network_name = "ICAD21_WIRE_CMPLX" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
