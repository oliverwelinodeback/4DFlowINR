...
def get_sweep_config():
    """
    Vincent factorial — 27-run grid: 3 cases × 3 data types × 3 optimizer settings.
    ...
    Factors:
      ...
      optimizer: R4 (long-LBFGS, no SA), R3 (SA-Adam only), R2 (SA + LBFGS)
...
def get_config(sweep_config=None):
    """
    Vincent factorial base config.

    Base data: ICAD48 LR (placeholder — all data/venc/resolution overridden by
    trainer.py SWEEP_VINCENT routing block per run_id).

    Architecture: identical to Sweep N (omega_0=30, sigma_0=30, out_dim=6, depth=6,
    hidden=128). No meta-init for any run.
    """
    ...
    # MODEL
    # ==========================================
    config.networks_folder = "../models/260505_Vincent_factorial/"
    config.network_name = "260505_Vincent"
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
