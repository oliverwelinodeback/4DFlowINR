...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-GAUSS_bias_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'network.sigma_0': {'values': [1, 5, 10, 30, 50, 70, 100, 150, 200, 400]},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251010_WIRE_GAUSS_sweep/"
    config.network_name = "HV01_WIRE_GAUSS" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
