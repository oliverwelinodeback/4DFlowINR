...
def get_sweep_config():
    ...
    return {
        'name': f'WIRE-SIREN_bias_sweep_{timestamp}', 
        'method': 'grid',
        'metric': {'name': 'FINAL Relative Error [Fluid]', 'goal': 'minimize'},
        'parameters': {
            'network.omega_0': {'values': [5, 10, 30, 50, 70, 100, 150, 200, 400]},
        },
    }
...
def get_config():
    ...
    # Model 
    config.networks_folder = "../models/251010_WIRE_SIREN_sweep/"
    config.network_name = "HV01_WIRE_SIREN" 
    timestamp = datetime.now().strftime('%Y%m%d-%H%M')
