# --- Vincent factorial evaluation: metrics + residuals + c_weights -----------
#
# Evaluates all 27 Vincent factorial runs across 3 cases:
#   - Velocity relative error          [%]  — whole domain + stenosis (ICAD48/ICAD21)
...
#
# Workflow:
#   1. Run plot_stenosis_mask_check_vincent.py for ICAD48 and ICAD21 to find box coords
#   2. Fill in the stenosis_box dicts below with the confirmed coordinates
#   3. Run: python tools/evaluate_vincent_metrics.py
#
# NOTE: LR and HRLR share the same normalised coordinate system (use_baseline_normalization=True
# means the template-derived mean/std is used, not the data-grid mean/std). One stenosis_box
# per case works for all data types. Data is loaded once per (case, dtype) — 6 loads total —
# because the input file and ref_spatial_factor differ between LR and HRLR.
#
# Run from src/:  python tools/evaluate_vincent_metrics.py
# ---------------------------------------------------------------------------
import os
import sys
...
from tools.plot_PINN_PDE_residuals import plot_c_weights_spatial, plot_residual_distribution


# ---------------------------------------------------------------------------
# Settings
# ---------------------------------------------------------------------------
MODEL_DIR = "../models/260505_Vincent_factorial"
CKPT_ITER = 30_000
...
OUT_DIR   = "residuals/Vincent_eval"

# ---------------------------------------------------------------------------
# Case definitions — grouped by (case, dtype)
# ---------------------------------------------------------------------------
# stenosis_box: fill in after running plot_stenosis_mask_check_vincent.py.
...
CASE_RUNS = {
    "HV01": {
        "dtype_meta": {
            "LR": dict(
                data_file     = "../data/Vincent/healthy/HV01_05mm3_20ms_LR_sv17_tSNR10_newMask.h5",
                data_file_ref = "../data/Vincent/healthy/HV01_05mm3_20ms.h5",
                venc=1.7, peak_flow_idx=12,
                ref_spatial_factor=2, ref_temporal_factor=2,
                dx=0.001, dt=0.04,
            ),
            "HRLR": dict(
                data_file     = "../data/Vincent/healthy/HV01_05mm3_20ms_HRLR_sv17_tSNR10.h5",
                data_file_ref = "../data/Vincent/healthy/HV01_05mm3_20ms.h5",
                venc=1.7, peak_flow_idx=12,
        ...
        "runs": {
            "LR_LBFGS":        (os.path.join(MODEL_DIR, "SA_PINN_HV01_LR_LBFGS"),        "LR"),
            "LR_SA-Adam":      (os.path.join(MODEL_DIR, "SA_PINN_HV01_LR_SA-Adam"),       "LR"),
            "LR_SA+LBFGS":     (os.path.join(MODEL_DIR, "SA_PINN_HV01_LR_SA+LBFGS"),     "LR"),
            "HRLR10_LBFGS":    (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR10_LBFGS"),    "HRLR"),
            "HRLR10_SA-Adam":  (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR10_SA-Adam"),  "HRLR"),
            "HRLR10_SA+LBFGS": (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR10_SA+LBFGS"), "HRLR"),
            "HRLR2_LBFGS":     (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR2_LBFGS"),    "HRLR"),
            "HRLR2_SA-Adam":   (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR2_SA-Adam"),   "HRLR"),
            "HRLR2_SA+LBFGS":  (os.path.join(MODEL_DIR, "SA_PINN_HV01_HRLR2_SA+LBFGS"), "HRLR"),
        },
    },
    "ICAD48": {
        "dtype_meta": {
...
ckpt_filename = f"260505_Vincent_it{CKPT_ITER:06d}.pth"
...
for case_name, case_cfg in CASE_RUNS.items():
    ...
    # Per-run evaluation
    # ------------------------------------------------------------------
    for run_name, (run_dir, dtype) in runs.items():
        print(f"\n{'='*55}")
        print(f"  {case_name} / {run_name}  [{dtype}]")
        print('='*55)

        ckpt_path = os.path.join(run_dir, "checkpoints", ckpt_filename)
