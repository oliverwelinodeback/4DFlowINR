...
if __name__ == '__main__':
    ...
    outputLR_filename = f'{base_path}/{case_name}_LR_dv_tSNR8_test.h5'
    ...
    #for idx in range(0, data_count): # Temporal downsample!
    #for idx in range(0, data_count, 3): # Temporal downsample!
    for idx in range(0, 4, 3): # test
        ...
        # DO the downsampling
        lr_u_lv, mag = fft.downsample_phase_img(hr_u, mag_image, low_venc, crop_ratio, targetSNR)
        lr_v_lv, _ = fft.downsample_phase_img(hr_v, mag_image, low_venc, crop_ratio, targetSNR)
        lr_w_lv, _ = fft.downsample_phase_img(hr_w, mag_image, low_venc, crop_ratio, targetSNR)

        lr_u_hv, _ = fft.downsample_phase_img(hr_u, mag_image, high_venc, crop_ratio, targetSNR)
        lr_v_hv, _ = fft.downsample_phase_img(hr_v, mag_image, high_venc, crop_ratio, targetSNR)
        lr_w_hv, _ = fft.downsample_phase_img(hr_w, mag_image, high_venc, crop_ratio, targetSNR)
